...
<template>
  <div class="p-4">
    ...
    <div class="flex justify-between mt-1 text-sm">
      ...
      <span v-else class="text-gray-500"> {{ remainingChars }} remaining </span>
    </div>
  </div>
</template>
```
...
<template>
  <div class="p-4 max-w-md">
    ...
    <div class="flex gap-2 mb-4">
      ...
      />
      <button @click="addScore" class="bg-blue-500 text-white px-4 rounded">
        Add
      </button>
    </div>
...
const roundedFahrenheit = computed(
  () => Math.round(fahrenheit.value * 100) / 100
);
</script>
  ...
  <div class="max-w-xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden">
    <div class="bg-gradient-to-r from-blue-500 to-purple-600 p-6 text-white">
      <h2 class="text-xl font-bold">
        {{ selectedPlatform.icon }} {{ selectedPlatform.name }} Composer
      </h2>
    ...
    <div class="p-6">
      ...
      <div class="flex justify-between items-center text-sm mb-4">
        ...
        <span class="text-gray-400"> {{ charCount }} / {{ maxChars }} </span>
      </div>

      <div
        class="grid grid-cols-2 gap-4 text-sm text-gray-600 mb-6 bg-gray-50 p-4 rounded-lg"
      >
        <div><span class="font-medium">Words:</span> {{ wordCount }}</div>
        <div>
...
_Reply 'next' for Lesson 5.1 (Theory)._
